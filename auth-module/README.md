...
> Модуль аутентификации и авторизации пользователей для **Rent Apartment App**.  
> На текущем этапе является наиболее оформленным showcase-ready модулем проекта и может использоваться как отдельный backend-сервис или как часть монорепозитория.
...
- Swagger UI для проверки и документирования API.
- Инициализация базы данных с `SUPER_ADMIN` по умолчанию.

Этот модуль покрывает типовой backend-сценарий аутентификации и авторизации для веб-сервиса: регистрация, вход, роли, токены и защищённые административные действия.
...
"код отправлен"
~~~
В текущей showcase-конфигурации отправка письма реализована через mock `email-sender`: код подтверждения логируется в консоль и используется при авторизации в поле `code`.
...
---
## 💬 Примечание
`auth-module` может использоваться как автономный backend-сервис аутентификации и авторизации
или как часть общей архитектуры `Rent Apartment App`.

При интеграции с другими модулями монорепозитория (`rent_module`, `email-sender`, `gateway`, `eureka-server`)
он регистрируется в Eureka и взаимодействует с остальными сервисами через API Gateway.

На текущем этапе модуль является наиболее оформленной showcase-ready частью проекта и используется как основной публичный пример backend-реализации.
